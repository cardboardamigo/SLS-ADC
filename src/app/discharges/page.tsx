...
import { addDischarge, getDischargesForMonth, deleteDischarge } from "@/lib/census";
...
export default function DischargesPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      console.error("Failed to add discharge:", err);
    } finally {
      setSubmitting(false);
    }
  }
    ...
    <div className="min-h-screen bg-gray-50 pb-20 pt-14">
      ...
      <div className="max-w-lg mx-auto px-4 py-4">
        ...
        <form onSubmit={handleSubmit} className="bg-white rounded-2xl p-5 shadow-sm border border-gray-100 mb-4">
          <h2 className="font-semibold text-gray-800 mb-4">Record Discharge</h2>
          ...
          >
            {submitting ? "Saving..." : "Record Discharge"}
          </button>
        </form>
        ...
        <div className="bg-white rounded-2xl p-5 shadow-sm border border-gray-100">
          <h2 className="font-semibold text-gray-800 mb-3">
            This Month&apos;s Discharges ({recentDischarges.length})
          </h2>
          {recentDischarges.length === 0 ? (
            <p className="text-gray-400 text-sm text-center py-4">No discharges this month</p>
          ) : (
            <div className="space-y-2 max-h-64 overflow-y-auto">
              {recentDischarges.map((d) => (
                ...
                >
                  <div>
                    <p className="text-sm font-medium text-gray-800">{d.dischargeName}</p>
